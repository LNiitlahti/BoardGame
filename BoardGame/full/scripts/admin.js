...
async function confirmMassImport(triggerBtn) {
    ...
    // Queue entries get renumbered from startMatchNumber, but linkedMatch
    // references still carry the export file's own numbering — remap them,
    // or split-format pairs silently stop resolving whenever the queue
    // wasn't empty at import time (see _tagImportedBatch's partner lookup).
    const matchNumberMap = new Map();
    matches.forEach((m, i) => {
        if (m.matchNumber !== undefined) matchNumberMap.set(m.matchNumber, startMatchNumber + i);
    });
    ...
    for (const match of matches) {
        ...
        const queueEntry = {
            ...
            linkedMatch: (match.linkedMatch !== undefined && match.linkedMatch !== null)
                ? (matchNumberMap.get(match.linkedMatch) ?? null)
                : null,
...
/**
 * Keep-predicate for Clear Queue. Spares completed history and live matches
 * always; spares future-round scheduled matches (the mass-imported schedule)
 * unless removeFutureRounds is set — that opt-in exists so a corrected
 * schedule can be re-imported without duplicating the old one.
 */
function clearQueueKeepPredicate(removeFutureRounds) {
...
/**
 * Render the Clear Queue preview counts from the SAME predicate
 * confirmClearQueue uses, so the shown numbers match what actually happens.
 * Re-runs when the "also remove future rounds" checkbox is toggled.
 */
function updateClearQueuePreview() {
    const previewContainer = document.getElementById('clearQueuePreview');
    if (!previewContainer) return;

    const removeFuture = document.getElementById('clearFutureRounds')?.checked || false;
    const keep = clearQueueKeepPredicate(removeFuture);
    const removable = (gameState?.gameQueue || []).filter(g => !keep(g));

    if (removable.length === 0) {
        previewContainer.innerHTML = '<p>Nothing to remove with the current selection.</p>';
        return;
    }

    const currentRound = gameState.currentPhase?.roundNumber;
    const breakCount = removable.filter(g => g.isBreak).length;
    const futureCount = removable.filter(g => !g.isBreak &&
        g.roundNumber !== undefined && currentRound !== undefined && g.roundNumber > currentRound).length;
    const pendingCount = removable.length - breakCount - futureCount;

    previewContainer.innerHTML = `
        <p><strong>${removable.length}</strong> entr${removable.length !== 1 ? 'ies' : 'y'} will be removed:</p>
        <ul style="margin: 8px 0; padding-left: 20px; color: var(--text-secondary);">
            ...
            ${futureCount > 0 ? `<li>${futureCount} future-round scheduled match${futureCount !== 1 ? 'es' : ''}</li>` : ''}
            ${breakCount > 0 ? `<li>${breakCount} break${breakCount !== 1 ? 's' : ''}</li>` : ''}
        </ul>
    `;
}
...
function openClearQueueModal() {
    ...
    // Anything beyond completed history / live matches is at least
    // potentially removable (future rounds require the checkbox).
    const anyRemovable = (gameState.gameQueue || [])
        .some(g => g.status !== 'completed' && g.status !== 'ongoing');
    if (!anyRemovable) {
        showStatus('No matches in queue to clear', 'warning');
        return;
    }

    const futureCheckbox = document.getElementById('clearFutureRounds');
    if (futureCheckbox) futureCheckbox.checked = false;
    updateClearQueuePreview();
...
async function confirmClearQueue(triggerBtn) {
    // Read the checkbox BEFORE closing the modal (close doesn't reset it,
    // but keep the read-then-close order obvious).
    const removeFuture = document.getElementById('clearFutureRounds')?.checked || false;
    closeClearQueueModal();

    // Spare: completed (history), ongoing (LIVE matches), and — unless the
    // admin opted in to removing them — matches tagged for future rounds
    // (the mass-imported schedule). "Clear All" means "clear this round's
...
/**
 * Award heart income to teams for the round that just ended.
 *
 * Flat per round: Mountain Heart +2, each Side Heart +1, for the hearts a
 * team still holds at payout time. A round in which no match was played pays
 * nothing. The calculation itself lives in calculateHeartIncome() in
 * shared/scripts/board-module.js — this function only accumulates the result
 * onto team.points. See docs/architecture/scoring.md.
 *
 * Returns object with points awarded per team NAME, for display.
 */
function awardRoundPoints() {
    if (!gameState?.teams || !boardModule) {
        console.warn('[scoring] awardRoundPoints: no gameState.teams or boardModule — no income paid');
        return {};
    }
    ...
    gameState.teams.forEach(team => {
        const roundPoints = byTeam[team.id]?.points || 0;
    ...
    // Expose the round's match count so callers can say "no matches played"
    // in the payout message — a silently unpaid round must be visible.
    awardRoundPoints.lastMatchesPlayed = matchesPlayed;
...
function openMatchSchedulerWindow() {
    let url = '../dev/dev-matchscheduler.html';
    if (currentTournamentId) {
